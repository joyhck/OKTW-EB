...
namespace LeagueSharp.Common
{
    ...
    public static class Orbwalking
    {
        ...
        public class Orbwalker
        {
            ...
            /// 
            private static Menu Menu, DrawMenu, MiscMenu, MissileMenu;

            public static int AALineWidth { get { return DrawMenu["AALineWidth"].Cast<Slider>().CurrentValue; } }
            public static bool LastHitHelper { get { return DrawMenu["LastHitHelper"].Cast<CheckBox>().CurrentValue; } }

            public static int HoldPosRadius { get { return MiscMenu["HoldPosRadius"].Cast<Slider>().CurrentValue; } }
            public static bool PriorizeFarm { get { return MiscMenu["PriorizeFarm"].Cast<CheckBox>().CurrentValue; } }
            public static bool AttackWards { get { return MiscMenu["AttackWards"].Cast<CheckBox>().CurrentValue; } }
            public static bool AttackPetsnTraps { get { return MiscMenu["AttackPetsnTraps"].Cast<CheckBox>().CurrentValue; } }
            public static bool AttackBarrel { get { return MiscMenu["AttackBarrel"].Cast<CheckBox>().CurrentValue; } }
            public static bool Smallminionsprio { get { return MiscMenu["Smallminionsprio"].Cast<CheckBox>().CurrentValue; } }
            public static bool _LimitAttackSpeed { get { return MiscMenu["LimitAttackSpeed"].Cast<CheckBox>().CurrentValue; } }
            public static bool FocusMinionsOverTurrets { get { return MiscMenu["FocusMinionsOverTurrets"].Cast<KeyBind>().CurrentValue; } }

            public static bool _MissileCheck { get { return Menu["MissileCheck"].Cast<CheckBox>().CurrentValue; } }

            public static int ExtraWindup { get { return Menu["ExtraWindup"].Cast<Slider>().CurrentValue; } }
            public static int _FarmDelay { get { return Menu["FarmDelay"].Cast<Slider>().CurrentValue; } }

            public static bool LastHit { get { return Menu["LastHit"].Cast<KeyBind>().CurrentValue; } }
            public static bool Farm { get { return Menu["Farm"].Cast<KeyBind>().CurrentValue; } }
            public static bool Freeze { get { return Menu["Freeze"].Cast<KeyBind>().CurrentValue; } }
            public static bool LaneClear { get { return Menu["LaneClear"].Cast<KeyBind>().CurrentValue; } }
            public static bool Orbwalk { get { return Menu["Orbwalk"].Cast<KeyBind>().CurrentValue; } }
            public static bool StillCombo { get { return Menu["StillCombo"].Cast<KeyBind>().CurrentValue; } }

            public Orbwalker()
            {
                Menu = MainMenu.AddMenu("L# Common", "L#");

                /* Drawings submenu */
                DrawMenu = Menu.AddSubMenu("Drawings", "drawings");
                DrawMenu.Add("AALineWidth", new Slider("Line Width", 2, 1, 6));
                DrawMenu.Add("LastHitHelper", new CheckBox("Last Hit Helper", false));

                /* Misc options */
                MiscMenu = Menu.AddSubMenu("Misc", "Misc");
                MiscMenu.Add("HoldPosRadius", new Slider("Hold Position Radius", 0, 0, 250));
                MiscMenu.Add("PriorizeFarm", new CheckBox("Priorize farm over harass"));
                MiscMenu.Add("AttackWards", new CheckBox("Auto attack wards", false));
                MiscMenu.Add("AttackPetsnTraps", new CheckBox("Auto attack pets & traps"));
                MiscMenu.Add("AttackBarrel", new CheckBox("Auto attack gangplank barrel"));
                MiscMenu.Add("Smallminionsprio", new CheckBox("Jungle clear small first", false));
                MiscMenu.Add("LimitAttackSpeed", new CheckBox("Don't kite if Attack Speed > 2.5", false));
                MiscMenu.Add("FocusMinionsOverTurrets", new KeyBind("Focus minions over objectives", false, KeyBind.BindTypes.PressToggle, 'M'));

                /* Missile check */
                Menu.Add("MissileCheck", new CheckBox("Use Missile Check", true));

                /* Delay sliders */
                Menu.Add("ExtraWindup", new Slider("Extra windup time", 80, 0, 200));
                Menu.Add("FarmDelay", new Slider("Farm delay", 0, 0, 200));

                /*Load the menu*/
                Menu.Add("LastHit", new KeyBind("Last hit", false, KeyBind.BindTypes.HoldActive, 'X'));
                Menu.Add("Farm", new KeyBind("Mixed", false, KeyBind.BindTypes.HoldActive, 'C'));
                Menu.Add("Freeze", new KeyBind("Freeze", false, KeyBind.BindTypes.HoldActive, 'N'));
                Menu.Add("LaneClear", new KeyBind("LaneClear", false, KeyBind.BindTypes.HoldActive, 'V'));
                Menu.Add("Orbwalk", new KeyBind("Combo", false, KeyBind.BindTypes.HoldActive, 32));
                Menu.Add("StillCombo", new KeyBind("Combo without moving", false, KeyBind.BindTypes.HoldActive, 'N'));
            ...
            {
                get { return _MissileCheck; }
            }

            public static bool LimitAttackSpeed
            {
                get { return _LimitAttackSpeed; }
            }
            ...
            public virtual void RegisterCustomMode(string name, string displayname, uint key)
            {
                ...
                {
                    Menu.Add(
                        name, displayname).SetShared().SetValue(new KeyBind(key, KeyBindType.Press)));
                }
                 */
            }
            ...
            public virtual AttackableUnit GetTarget()
            {
                ...
                if ((mode == OrbwalkingMode.Mixed || mode == OrbwalkingMode.LaneClear) && !PriorizeFarm)
                {
                    var target = TargetSelector.GetTarget(-1f, DamageType.Physical);
                    ...
                    mode == OrbwalkingMode.Freeze)
                {
                    ...
                    foreach (var minion in MinionList)
                    {
                        ...
                        if (minion.Team == GameObjectTeam.Neutral && AttackBarrel && minion.CharData.BaseSkinName == "gangplankbarrel" && minion.IsHPBarRendered)
                        {
                            if (minion.Health < 2)
                            {
                                return minion;
                ...
                if (mode == OrbwalkingMode.LaneClear && (!FocusMinionsOverTurrets || !MinionManager.GetMinions(ObjectManager.Player.Position, GetRealAutoAttackRange(ObjectManager.Player)).Any()))
                {
                    /* turrets */
                ...
                {
                    var jminions = ObjectManager.Get<Obj_AI_Minion>().Where(mob => mob.IsValidTarget() && mob.Team == GameObjectTeam.Neutral && InAutoAttackRange(mob) && mob.CharData.BaseSkinName != "gangplankbarrel" && mob.Name != "WardCorpse");
                    result = Smallminionsprio ? jminions.MinOrDefault(mob => mob.MaxHealth) : jminions.MaxOrDefault(mob => mob.MaxHealth);
            ...
            private bool ShouldAttackMinion(Obj_AI_Minion minion, bool includeBarrel = false)
            {
                ...
                {
                    return AttackBarrel && minion.CharData.BaseSkinName == "gangplankbarrel" && minion.IsHPBarRendered;
                }
                ...
                return (AttackPetsnTraps || MinionManager.IsMinion(minion)) && minion.CharData.BaseSkinName != "gangplankbarrel";
            }
            ...
            private void GameOnOnGameUpdate(EventArgs args)
            {
                ...
                Orbwalk(target, _orbwalkingPoint.To2D().IsValid() ? _orbwalkingPoint : Game.CursorPos, ExtraWindup, Math.Max(HoldPosRadius, 30));
            }
            ...
            private void DrawingOnOnDraw(EventArgs args)
            {
                //if (AACircle)
                //{
                Render.Circle.DrawCircle(Player.Position, GetRealAutoAttackRange(null) + 65, Color.Red, AALineWidth);
                //}
                //if (AACircle2)
                //{
