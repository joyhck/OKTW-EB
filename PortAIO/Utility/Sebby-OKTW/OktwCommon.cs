    ...
    public class OktwCommon
    {
        ...
        public static bool IsSpellHeroCollision(AIHeroClient t, LeagueSharp.Common.Spell QWER, int extraWith = 50)
        {
            foreach (var hero in HeroManager.Enemies.FindAll(hero => hero.IsValidTarget(QWER.Range + QWER.Width, true, QWER.RangeCheckFrom) && t.NetworkId != hero.NetworkId))
            {
                ...
                if (prediction.UnitPosition.To2D().LSDistance(QWER.From.To2D(), QWER.GetPrediction(t).CastPosition.To2D(), true, true) <= powCalc)
                {
                    return true;
                }
                else if (prediction.UnitPosition.To2D().LSDistance(QWER.From.To2D(), t.ServerPosition.To2D(), true, true) <= powCalc)
                {
                    return true;
                }
        ...
        public static bool CollisionYasuo(Vector3 from, Vector3 to)
        {
            ...
            var wallDirection = (yasuoWall.CastPosition.To2D() - yasuoWall.YasuoPosition.To2D()).LSNormalized().Perpendicular();
        ...
        private static void Obj_AI_Base_OnDoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (args.Target != null && args.SData != null)
            {
                ...
                {
                    IncomingDamageList.Add(new UnitIncomingDamage { Damage = sender.LSGetSpellDamage((AIHeroClient)args.Target, args.SData.Name), TargetNetworkId = args.Target.NetworkId, Time = Game.Time, Skillshot = false });
                }
            }
        }
