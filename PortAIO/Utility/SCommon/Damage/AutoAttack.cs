    ...
    public static class AutoAttack
    {
        ...
        static AutoAttack()
        {
            ...
            ItemPassives.Add(
                ...
                {
                    ItemID = 3115,
                    GetDamage = (s, t) => (float)s.CalcDamage(t, DamageType.Magical, 15 + ObjectManager.Player.AbilityPower() * 15 / 100)
                });
            ...
            AAEnpowers[Data.GetID("Ekko")].Add(
                new AAEnpower
                {
                    ...
                    GetDamage = (source, target) =>
                        {
                            float dmg = (float)source.CalcDamage(target, DamageType.Magical, (target.MaxHealth - target.Health) * (5 + Math.Floor(source.AbilityPower() / 100) * 2.2f) / 100);
            ...
            AAEnpowers[Data.GetID("Rengar")].Add(
                new AAEnpower
                {
                    ...
                    GetDamage = (source, target) => (float)source.CalcDamage(target, DamageType.Physical, new int[] { 30, 60, 90, 120, 150 }[source.GetSpell(SpellSlot.Q).Level - 1] + (source.BaseAttackDamage + source.FlatPhysicalDamageMod) * new int[] { 0, 5, 10, 15, 20 }[source.GetSpell(SpellSlot.Q).Level - 1] / 100f)
                });
            ...
            AAEnpowers[Data.GetID("Rengar")].Add(
                new AAEnpower
                {
                    ...
                    GetDamage = (source, target) => (float)source.CalcDamage(target, DamageType.Physical, new int[] { 30, 45, 60, 75, 90, 105, 120, 135, 150, 160, 170, 180, 190, 200, 210, 220, 230, 240 }[source.Level - 1] + (source.BaseAttackDamage + source.FlatPhysicalDamageMod) * 0.3f)
                });
            ...
            AAEnpowers[Data.GetID("TwistedFate")].Add(
                new AAEnpower
                {
                        ...
                        GetDamage = (source, target) => (float)Math.Floor(source.CalcDamage(target, DamageType.Magical, new int[] { 55, 80, 105, 130, 155 }[source.Spellbook.GetSpell(SpellSlot.E).Level - 1] + source.AbilityPower() * 0.5f)),
                });
        ...
        public static float GetDamage(Obj_AI_Base target, bool passiveCheck = false)
        {
            ...
            if (target.Type == GameObjectType.obj_AI_Minion)
                ...
                dmg = (float)ObjectManager.Player.CalcDamage(target, DamageType.Physical, k + (passiveCheck ? PassiveFlatMod(ObjectManager.Player, target) : 0));
            if (hero.ChampionName == "Azir")
            {
                ...
                if (cnt > 0)
                {
                    dmg = (float)hero.CalcDamage(target, DamageType.Magical, new[] { 50, 55, 60, 65, 70, 75, 80, 85, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180 }[ObjectManager.Player.Level - 1] + 0.6f * hero.AbilityPower());
