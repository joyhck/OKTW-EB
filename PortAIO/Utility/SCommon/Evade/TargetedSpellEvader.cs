    ...
    public class TargetedSpellEvader
    {
        ...
        public TargetedSpellEvader(Action<DetectedTargetedSpellArgs> fn, Menu menuToAttach)
        {
            ...
            foreach (var enemy in HeroManager.Enemies)
            {
                ...
                {
                    m_Menu.Add(String.Format("SCommon.TargetedSpellEvader.Spell.{0}", spell.SpellName), new CheckBox(String.Format("{0} ({1})", spell.ChampionName, spell.Slot) + (spell.IsDangerous ? " (Dangerous)" : "")));
                }
            }
