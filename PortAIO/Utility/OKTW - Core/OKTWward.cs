...
namespace OneKeyToWin_AIO_Sebby.Core
{
    ...
    class OKTWward
    {
        ...
        private void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                Item itm = new Item(ItemId.Farsight_Alteration);
                itm.Buy();
            }
        ...
        private void AutoWardLogic()
        {
            foreach (var enemy in Program.Enemies.Where(enemy => enemy.IsValid && !enemy.IsVisible && !enemy.IsDead))
            {
                ...
                if (timer > 1 && timer < 3)
                {
                    ...
                    //if (PPDistance < 800 && Player.ChampionName == "Caitlyn" && W.IsReady() && Player.Mana > 200f && Config.Item("bushW", true).GetValue<bool>() && Utils.TickCount - W.LastCastAttemptT > 2000)
                    //{
