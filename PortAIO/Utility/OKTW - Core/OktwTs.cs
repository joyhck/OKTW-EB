...
namespace OneKeyToWin_AIO_Sebby.Core
{
    class OktwTs
    {
        ...
        public void LoadOKTW()
        {
            ...
            foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsEnemy).OrderBy(enemy => enemy.MaxHealth / Player.GetAutoAttackDamage(enemy)))
            {
                Sub.Add("TsAaPriority" + enemy.ChampionName, new Slider(enemy.ChampionName, i, 0, 5));
                i--;
            }
            EloBuddy.SDK.Orbwalker.OnPreAttack += Orbwalking_BeforeAttack;
            EloBuddy.SDK.Orbwalker.OnPostAttack += Orbwalking_AfterAttack;
        ...
        private void Orbwalking_BeforeAttack(AttackableUnit target, Orbwalker.PreAttackArgs args)
        {
            ...
            if (args.Target is AIHeroClient)
            {
                ...
                if (forceFocusEnemy.NetworkId != newTarget.NetworkId && Game.Time - LatFocusTime < getSliderItem("extraTime") / 1000)
                {
                    args.Process = false;
        ...
        private void OnDraw(EventArgs args)
        {
            ...
            {
                LeagueSharp.Common.Utility.DrawCircle(Player.Position, Player.AttackRange + Player.BoundingRadius + getSliderItem("extraRang"), System.Drawing.Color.Gray, 1, 1);
        ...
        {
            if (getSliderItem("TsAa") == 2 || !SebbyLib.Orbwalking.CanAttack() || !Program.Combo)
            {
                return;
            }
            ...
            if (orbT != null)
            {
                ...
                {
                    foreach (var enemy in Program.Enemies.Where(enemy => enemy.IsValidTarget() && LeagueSharp.Common.Orbwalking.InAutoAttackRange(enemy)))
                    {
                ...
                else
                {
                    foreach (var enemy in Program.Enemies.Where(enemy => enemy.IsValidTarget() && LeagueSharp.Common.Orbwalking.InAutoAttackRange(enemy)))
                    {
                        ...
                        if (getSliderItem("TsAaPriority" + enemy.ChampionName) > getSliderItem("TsAaPriority" + bestTarget.ChampionName))
                        {
                            bestTarget = enemy;
                        }
