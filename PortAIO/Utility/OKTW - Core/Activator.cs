    ...
    class Activator
    {
        ...
        public void LoadOKTW()
        {
            ...
            foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.Team != Player.Team))
            {
                ...
                Sub.Add("spellZ" + spell.SData.Name, new CheckBox(enemy.ChampionName + ": " + spell.Name, spell.SData.TargettingType == SpellDataTargetType.Unit));
            }
            ...
            Sub.Add("cleanHP", new Slider("Use only under % HP", 80, 0, 100));
        ...
        {
            if (getCheckBoxItem("HydraTitanic") && Program.Combo && HydraTitanic.IsReady() && target.IsValid<AIHeroClient>())
            {
                HydraTitanic.Cast();
        ...
        private void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            {
                if (getCheckBoxItem("spellZ" + args.SData.Name) != null && getCheckBoxItem("spellZ" + args.SData.Name))
                {
        ...
        private void Survival()
        {
            ...
            foreach (var ally in Program.Allies.Where(ally => ally.IsValid && !ally.IsDead && ally.HealthPercent < 50 && Player.Distance(ally.ServerPosition) < 700))
            {
                ...
                if (getCheckBoxItem("FaceOfTheMountain") && FaceOfTheMountain.IsReady() && Player.Distance(ally.ServerPosition) < FaceOfTheMountain.Range)
                {
            ...
            if (CanUse(smite))
            {
                ...
                if (mobs.Count == 0 && (Player.GetSpellSlot("s5_summonersmiteplayerganker") != SpellSlot.Unknown || Player.GetSpellSlot("s5_summonersmiteduel") != SpellSlot.Unknown))
                {
                    ...
                    if (enemy.IsValidTarget())
                    {
                        ...
                        if (getCheckBoxItem("SmiteEnemyKS") && enemy.Health - OktwCommon.GetIncomingDamage(enemy) < smiteDmg)
                            Player.Spellbook.CastSpell(smite, enemy);
                    }
                }
        ...
        private void Cleansers()
        {
            ...
            if (Player.HealthPercent >= (float)getSliderItem("cleanHP") || !getCheckBoxItem("Clean"))
                return;
            ...
            if (Mikaels.IsReady())
            {
                foreach (var ally in Program.Allies.Where(
                    ally => ally.IsValid && !ally.IsDead && getCheckBoxItem("MikaelsAlly" + ally.ChampionName) && Player.Distance(ally.Position) < Mikaels.Range
                    && ally.HealthPercent < (float)getSliderItem("cleanHP")))
                {
                    ...
                    if (ally.HasBuffOfType(BuffType.Taunt) && getCheckBoxItem("Taunt")) // getCheckBoxItem("Taunt")
                        Mikaels.Cast(ally);
                    if (ally.HasBuffOfType(BuffType.Suppression) && getCheckBoxItem("Suppression")) // getCheckBoxItem("Suppression")
                        Mikaels.Cast(ally);
        ...
        private void Clean()
        {
            if (Quicksilver.IsReady())
                LeagueSharp.Common.Utility.DelayAction.Add(getSliderItem("CSSdelay"), () => Quicksilver.Cast());
            else if (Mercurial.IsReady())
                LeagueSharp.Common.Utility.DelayAction.Add(getSliderItem("CSSdelay"), () => Mercurial.Cast());
            else if (Dervish.IsReady())
                LeagueSharp.Common.Utility.DelayAction.Add(getSliderItem("CSSdelay"), () => Dervish.Cast());
            else if (cleanse != SpellSlot.Unknown && cleanse.IsReady() && getCheckBoxItem("Cleanse"))
                LeagueSharp.Common.Utility.DelayAction.Add(getSliderItem("CSSdelay"), () => Player.Spellbook.CastSpell(cleanse, Player));
        }
