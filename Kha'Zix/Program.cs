...
namespace KhaZix
{
    public static class Program
    {
        ...
        public static bool Kson { get { return Menu["Kson"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseQKs { get { return Menu["UseQKs"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseWKs { get { return Menu["UseWKs"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseEKs { get { return Menu["UseEKs"].Cast<CheckBox>().CurrentValue; } }
        public static bool Ksbypass { get { return Menu["Ksbypass"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseEQKs { get { return Menu["UseEQKs"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseEWKs { get { return Menu["UseEWKs"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseTiamatKs { get { return Menu["UseTiamatKs"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseIgnite { get { return Menu["UseIgnite"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseQHarass { get { return Menu["UseQHarass"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseWHarass { get { return Menu["UseWHarass"].Cast<CheckBox>().CurrentValue; } }
        public static bool HarassAutoWI { get { return Menu["Harass.AutoWI"].Cast<CheckBox>().CurrentValue; } }
        public static bool HarassAutoWD { get { return Menu["Harass.AutoWD"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseQFarm { get { return Menu["UseQFarm"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseEFarm { get { return Menu["UseEFarm"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseWFarm { get { return Menu["UseWFarm"].Cast<CheckBox>().CurrentValue; } }
        public static bool UseItemsFarm { get { return Menu["UseItemsFarm"].Cast<CheckBox>().CurrentValue; } }
        ...
        public static int Edelay { get { return Menu["Edelay"].Cast<Slider>().CurrentValue; } }
        public static int FarmWHealth { get { return Menu["Farm.WHealth"].Cast<Slider>().CurrentValue; } }
        ...
        private static void OnLoad(EventArgs args)
        {
            ...
            Menu.Add("Farm.WHealth", new Slider("Health % to use W", 80, 0, 100));
            Menu.Add("UseItemsFarm", new CheckBox("Use Items", true));
            Menu.AddSeparator();
        ...
        static Vector3 GetJumpPoint(AIHeroClient Qtarget, bool firstjump = true)
        {
            ...
            var jumptarget = IsHealthy ? HeroList.FirstOrDefault(x => x.IsValidTarget() && !x.IsZombie && x != Qtarget && Vector3.Distance(myHero.ServerPosition, x.ServerPosition) < E.Range) : HeroList.FirstOrDefault(x => x.IsAlly && !x.IsZombie && !x.IsDead && !x.IsMe && Vector3.Distance(myHero.ServerPosition, x.ServerPosition) < E.Range);
            if (jumptarget != null)
            {
        ...
        static void WaveClear()
        {
            ...
            {
                var minion = EntityManager.MinionsAndMonsters.GetLaneMinions().OrderBy(x => x.MaxHealth).FirstOrDefault();
                if (minion != null && Prediction.Health.GetPrediction(minion, (int)(myHero.Distance(minion) * 1000 / 1400)) > 0.35f * myHero.GetSpellDamage(minion, SpellSlot.Q) && myHero.Distance(minion) <= Q.Range)
                {
                    Q.Cast(minion);
                }
                else if (minion == null || !minion.IsValid)
                {
                    ...
                    {
                        if (Prediction.Health.GetPrediction(min, (int)(myHero.Distance(min) * 1000 / 1400)) > 3 * myHero.GetSpellDamage(min, SpellSlot.Q) && myHero.Distance(min) <= Q.Range)
                        {
                            Q.Cast(min);
                            break;
                        }
            ...
            if (UseWFarm && W.IsReady() && myHero.HealthPercent <= FarmWHealth)
            {
                ...
                var farmLocation = EntityManager.MinionsAndMonsters.GetCircularFarmLocation(wmins.ToList(), EvolvedW ? (float)WE.Width : (float)W.Width, EvolvedW ? (int)WE.Range : (int)W.Range);
                var distcheck = EvolvedW ? myHero.Distance(farmLocation.CastPosition) <= WE.Range : myHero.Distance(farmLocation.CastPosition) <= W.Range;
                if (distcheck)
                {
            ...
            {
                var farmLocation = EntityManager.MinionsAndMonsters.GetCircularFarmLocation(ObjectManager.Get<Obj_AI_Minion>().Where(x => x.IsValidTarget(E.Range)).ToList(), (float)E.Width, (int)E.Range);
            ...
            if (UseItemsFarm)
            {
                var farmLocation = EntityManager.MinionsAndMonsters.GetCircularFarmLocation(ObjectManager.Get<Obj_AI_Minion>().Where(x => x.IsValidTarget(400)).ToList(), 400, 400);

                if (Item.CanUseItem(ItemId.Ravenous_Hydra_Melee_Only) && myHero.Distance(farmLocation.CastPosition) <= 400 && farmLocation.HitNumber >= 2)
                {
                    Item.UseItem(ItemId.Ravenous_Hydra_Melee_Only);
                }
                if (Item.CanUseItem(ItemId.Tiamat_Melee_Only) && myHero.Distance(farmLocation.CastPosition) <= 400 && farmLocation.HitNumber >= 2)
                {
                    Item.UseItem(ItemId.Tiamat_Melee_Only);
                }
                if (Item.CanUseItem(ItemId.Titanic_Hydra) && myHero.Distance(farmLocation.CastPosition) <= 400 && farmLocation.HitNumber >= 2)
                {
        ...
        static void KillSteal()
        {
            ...
            if (target != null)
            {
                ...
                if (UseEKs && E.IsReady() && Vector3.Distance(myHero.ServerPosition, target.ServerPosition) <= E.Range && Vector3.Distance(myHero.ServerPosition, target.ServerPosition) > Q.Range)
                {
                ...
                if (W.IsReady() && !EvolvedW && Vector3.Distance(myHero.ServerPosition, target.ServerPosition) <= W.Range && UseWKs)
                {
                ...
                if (W.IsReady() && EvolvedW && Vector3.Distance(myHero.ServerPosition, target.ServerPosition) <= W.Range && UseWKs)
                {
                    ...
                    {
                        List<Obj_AI_Base> PCollision = pred.CollisionObjects.ToList();
                        var x = PCollision.FirstOrDefault(PredCollisionChar => Vector3.Distance(PredCollisionChar.ServerPosition, target.ServerPosition) <= 30);
                        if (x != null)
                        {
                            W.Cast(x.Position);
                            return;
                ...
                // MIXED EW KS
                if (W.IsReady() && E.IsReady() && !EvolvedW && Vector3.Distance(myHero.ServerPosition, target.ServerPosition) <= W.Range + E.Range && UseEWKs)
                {
                    double WDmg = myHero.GetSpellDamage(target, SpellSlot.W);
                    if (target.Health <= WDmg)
                    {
                ...
                if (Item.CanUseItem(ItemId.Tiamat_Melee_Only) && Vector2.Distance(myHero.ServerPosition.To2D(), target.ServerPosition.To2D()) <= 400 && UseTiamatKs)
                {
                ...
                if (Item.CanUseItem(ItemId.Ravenous_Hydra_Melee_Only) && Vector2.Distance(myHero.ServerPosition.To2D(), target.ServerPosition.To2D()) <= 400 && UseTiamatKs)
                {
